...
// GET /api/accounts — alle rekeningen (accounts.id = IBAN)
...
// GET /api/accounts/:iban — één rekening opzoeken op IBAN
router.get('/accounts/:iban', async (req, res) => {
  try {
    const [rows] = await pool.query(
      'SELECT id, owner_name, balance FROM accounts WHERE id = ?',
      [req.params.iban]
    );
    if (rows.length === 0)
      return res.status(404).json({
        ok: false, status: 404,
        code: C.ACCOUNT_UNKNOWN,
