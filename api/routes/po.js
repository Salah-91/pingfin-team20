...
const BIC = process.env.BIC || 'CEKVBE88';
const CB_URL = (process.env.CB_URL || 'https://stevenop.be/pingfin/api/v2').replace(/\/$/, '');
...
function validAmount(amount) {
  ...
  if (Math.round(n * 100) !== n * 100) return false; // max 2 decimalen
  return true;
}
...
function validatePoFields(po) {
  if (!validAmount(po.po_amount)) {
    return parseFloat(po.po_amount) <= 0
      ? { code: C.AMOUNT_INVALID, message: 'Bedrag moet positief zijn' }
      : { code: C.AMOUNT_EXCEEDED, message: 'Bedrag mag niet hoger zijn dan €500' };
  }
  if (po.ob_id && !validBic(po.ob_id))
    return { code: C.BB_UNKNOWN, message: 'Ongeldige OB BIC-formaat' };
  if (po.bb_id && !validBic(po.bb_id))
    return { code: C.BB_UNKNOWN, message: 'Ongeldige BB BIC-formaat' };
  if (po.oa_id && !validIban(po.oa_id))
    return { code: C.ACCOUNT_UNKNOWN, message: 'Ongeldig IBAN-formaat voor OA' };
  if (po.ba_id && !validIban(po.ba_id))
    return { code: C.ACCOUNT_UNKNOWN, message: 'Ongeldig IBAN-formaat voor BA' };
  return null;
}
...
router.get('/po_new/generate', async (req, res) => {
  try {
    ...
    await writeLog(null, 'error', `po_new/generate fout: ${err.message}`);
...
router.post('/po_new/add', async (req, res) => {
  try {
    ...
    await writeLog(null, 'error', `po_new/add fout: ${err.message}`);
    res.status(500).json({ ok: false, status: 500, code: null, message: err.message, data: null });
  }
});

// ─── POST /api/po_new/manual — manuele PO aanmaken ──────────────────────────

router.post('/po_new/manual', async (req, res) => {
  try {
    const { po_amount, po_message, oa_id, ob_id, ba_id, bb_id } = req.body;

    const validErr = validatePoFields({ po_amount, oa_id, ba_id, ob_id: ob_id || BIC, bb_id });
    if (validErr) {
      await writeLog(null, 'validation_error', `Manuele PO geweigerd: ${validErr.message}`);
      return res.status(400).json({ ok: false, status: 400, code: validErr.code, message: validErr.message, data: null });
    }

    if (!oa_id) return res.status(400).json({ ok: false, status: 400, code: C.ACCOUNT_UNKNOWN, message: 'oa_id is verplicht', data: null });
    if (!ba_id) return res.status(400).json({ ok: false, status: 400, code: C.ACCOUNT_UNKNOWN, message: 'ba_id is verplicht', data: null });
    if (!bb_id) return res.status(400).json({ ok: false, status: 400, code: C.BB_UNKNOWN, message: 'bb_id is verplicht', data: null });
    ...
    await writeLog(null, 'error', `po_new/manual fout: ${err.message}`);
    res.status(500).json({ ok: false, status: 500, code: null, message: err.message, data: null });
  }
});
  ...
  try {
    ...
    for (const po of pos) {
      ...
      if (parseFloat(po.po_amount) <= 0 || isNaN(parseFloat(po.po_amount))) {
        ...
        if (oa.length === 0) {
          errorCode = C.ACCOUNT_UNKNOWN;
        } else if (parseFloat(oa[0].balance) < parseFloat(po.po_amount)) {
          errorCode = C.INSUFFICIENT_BALANCE;
        }
      }
      ...
      if (isInternal) {
        ...
        if (ba.length === 0) {
          ...
          await pool.query('DELETE FROM po_new WHERE po_id = ?', [po.po_id]);
          results.push({ po_id: po.po_id, status: 'REJECTED', code: C.ACCOUNT_UNKNOWN, message: 'BA onbekend' });
          continue;
        }
        ...
        await pool.query('DELETE FROM po_new WHERE po_id = ?', [po.po_id]);
        await writeLog(po.po_id, 'po_internal', `Interne betaling verwerkt: ${po.oa_id} → ${po.ba_id} €${po.po_amount}`);
...
// ─── POST /api/po_in — ontvang inkomende PO van CB (wij = BB) ───────────────
...
router.post('/po_in', async (req, res) => {
  ...
  for (const po of poList) {
    ...
    // Crediteer BA en registreer alles
    try {
